    ...
    <section className="cta-band">
      <div className="shell">
        ...
        <Link className="button button--white" href={pageHref(locale, page)}>
          {label} <MaterialIcon name="arrow_forward" />
        </Link>
      </div>
    </section>
  );
...
function InfoCard({
  ...
  return (
    <article className={`info-card${accent ? " info-card--teal" : ""}`}>
      ...
      <section className="hero hero--cinematic">
        ...
        <div className="shell hero-cinematic__content">
          <div className="hero-copy">
            ...
            <div className="event-meta">
              <div>
                <span className="meta-icon">24</span>
                <p>
          ...
          <div className="hero-slide-note">
            <span>01</span>
            <div>
              <b>{pageCopy.slideNoteTitle}</b>
              <small>{pageCopy.slideNoteText}</small>
            </div>
          </div>
        </div>
        <div className="shell sector-bar sector-bar--overlay">
          ...
          {pageCopy.sectors.map((sector, index) => (
            <Link href={pageHref(locale, "exhibitors")} key={sector}>
              <span>0{index + 1}</span>
              <b>{sector}</b>
            </Link>
          ))}
      ...
      <section className="section">
        <div className="shell">
          ...
          <div className="card-grid card-grid--4">
            ...
            <InfoCard accent number="04" title={pageCopy.ambition.cards[3].title} text={pageCopy.ambition.cards[3].text}>
...
function AboutPage({ locale }: PageProps) {
  ...
  return (
    <>
      ...
      <section className="section section--soft"><div className="shell"><div className="section-heading section-heading--center"><p className="kicker">{copy.expect.kicker}</p><h2>{copy.expect.title}</h2></div><div className="card-grid">{copy.expect.cards.map((card,index)=><InfoCard accent={index===copy.expect.cards.length-1} number={`0${index+1}`} title={card.title} text={card.text} key={card.title}/>)}</div></div></section>
      <section className="section"><div className="shell"><div className="stats-row">{copy.stats.map((stat) => <div className="stat" key={stat.label}><b>{stat.value}</b><span>{stat.label}</span></div>)}</div></div></section>
      <CtaBand locale={locale} title={copy.cta.title} text={copy.cta.text} page="participate" label={copy.cta.label} />
    </>
  );
}

function ExhibitorsPage({ locale, managedExhibitors }: PageProps & { managedExhibitors?: PublicExhibitor[] }) {
  ...
  return (
    <>
      ...
      <section className="section"><div className="shell"><ExhibitorDirectory exhibitors={directory} locale={locale} labels={{ search: copy.searchLabel, placeholder: copy.searchPlaceholder, all: copy.filterAll, empty: copy.empty, noMatches: copy.noMatches, resultCount: copy.resultCount }} /></div></section>
      <CtaBand locale={locale} title={copy.cta.title} text={copy.cta.text} page="participate" label={copy.cta.label} />
    </>
  );
}
...
function ExhibitorDetailPage({ locale, detail, managedExhibitors }: PageProps & { managedExhibitors?: PublicExhibitor[] }) {
  ...
  return (
    <>
      ...
      <section className="section"><div className="shell split"><div className="exhibitor-detail-logo"><span className={`fake-logo${matched.style ? ` fake-logo--${matched.style}` : ""}`}>{matched.letter}</span></div><div className="split-copy"><p className="kicker">{copy.exhibitorDetail.about.kicker}</p><h2>{copy.exhibitorDetail.about.title}</h2><p>{matched.text}</p><ul className="detail-list"><li><b>{copy.exhibitorDetail.categoryLabel}</b><span>{matched.category}</span></li>{matched.websiteUrl ? <li><b>{copy.exhibitorDetail.websiteLabel}</b><a href={matched.websiteUrl} target="_blank" rel="noopener noreferrer">{copy.common.visitWebsite}</a></li> : null}</ul></div></div></section>
...
function TicketsPage({ locale, event }: PageProps & { event?: PublishedEvent }) {
  ...
  return (
    <>
      ...
      <section className="section section--soft"><div className="shell"><div className="section-heading section-heading--center"><p className="kicker">{copy.visit.kicker}</p><h2>{copy.visit.title}</h2></div><div className="card-grid">{copy.visit.cards.map((card,index)=><InfoCard accent={index===2} number={`0${index+1}`} title={card.title} text={card.text} key={card.title}/>)}</div></div></section>
...
function VisitPage({ locale }: PageProps) {
  ...
  return (
    <>
      ...
      <section className="section section--soft"><div className="shell"><div className="section-heading"><div><p className="kicker">{copy.before.kicker}</p><h2>{copy.before.title}</h2></div></div><div className="card-grid card-grid--4">{copy.before.cards.map((card,index)=><InfoCard accent={index===3} number={`0${index+1}`} title={card.title} text={card.text} key={card.title}/>)}</div></div></section>
      <CtaBand locale={locale} title={copy.cta.title} text={copy.cta.text} page="contact" label={copy.cta.label}/>
    </>
  );
}
...
function FloorPlanPage({ locale, floorPlan }: PageProps & { floorPlan?: PublicFloorPlan | null }) {
  ...
  return (
    <>
      <PageHero locale={locale} title={copy.title} description={copy.description}/>
      <section className="section section--soft"><div className="shell"><div className="floor-plan-card">{isPdf ? <div className="media-frame"><p>{copy.currentPlan}: {floorPlan.originalName}</p><a className="button button--primary" href={planUrl} target="_blank" rel="noreferrer">{copy.openPdf} <MaterialIcon name="open_in_new" /></a></div> : floorPlan ? <img src={planUrl} alt={copy.imageAlt} /> : <Image src="/assets/expo-floor-plan.jpg" alt={copy.imageAlt} width={2000} height={1414}/>}<div className="legend">{copy.legend.map((item)=><span key={item}>{item}</span>)}</div><div className="floor-plan-actions"><a className="button button--primary" href={planUrl} target="_blank" rel="noreferrer">{copy.openLarge} <MaterialIcon name="open_in_new" /></a><a className="button button--outline" href={planUrl} target="_blank" rel="noreferrer" download>{copy.download} <MaterialIcon name="download" /></a></div></div></div></section>
      <CtaBand locale={locale} title={copy.cta.title} text={copy.cta.text} page="exhibitors" label={copy.cta.label}/>
    </>
  );
}
...
function FairMatchPage({ locale, managedExhibitors }: PageProps & { managedExhibitors?: PublicExhibitor[] }) {
  ...
  return (
    <>
      ...
      <section className="section"><div className="shell"><div className="section-heading section-heading--center"><p className="kicker">{copy.how.kicker}</p><h2>{copy.how.title}</h2><p>{copy.how.text}</p></div><div className="card-grid">{copy.how.cards.map((card,index)=><InfoCard accent={index===2} number={`0${index+1}`} title={card.title} text={card.text} key={card.title}/>)}</div></div></section>
      <section className="section section--soft"><div className="shell"><div className="section-heading"><div><p className="kicker">{copy.participants.kicker}</p><h2>{copy.participants.title}</h2></div><p>{copy.participants.text}</p></div>{directory.length ? <div className="clinic-grid">{directory.slice(0,5).map((item)=><article className="clinic-card" key={item.id}><div className="clinic-card__head"><span className={`fake-logo${item.style?` fake-logo--${item.style}`:""}`}>{item.letter}</span><div><small>{item.category}</small><h3>{item.name}</h3></div></div><p>{item.text}</p><Link className="button button--outline button--small" href={pageHref(locale, "exhibitor-detail", item.slug)}>{getDictionary(locale).common.viewProfile}</Link></article>)}</div> : <p className="demo-note">{copy.participants.empty}</p>}</div></section>
...
function ParticipatePage({ locale }: PageProps) {
  ...
  return (
    <>
      ...
      <section className="section section--cream" id="sponsorship"><div className="shell"><div className="section-heading"><div><p className="kicker">{copy.sponsorship.kicker}</p><h2>{copy.sponsorship.title}</h2></div><p>{copy.sponsorship.text}</p></div><div className="sponsor-grid">{copy.sponsorship.tiers.map((tier,index)=><article className={`sponsor-card sponsor-card--${["bronze","silver","gold"][index]}`} key={tier.name}><span className="sponsor-card__medal">{tier.name.slice(0,1)}</span><h3>{tier.name}</h3><p>{tier.text}</p></article>)}</div></div></section>
      ...
      <CtaBand locale={locale} title={copy.cta.title} text={copy.cta.text} page="participant-info" label={copy.cta.label}/>
    </>
  );
}
...
function ParticipantInfoPage({ locale }: PageProps) {
  ...
  return (
    <>
      ...
      <section className="section section--soft"><div className="shell"><div className="section-heading"><div><p className="kicker">{copy.documents.kicker}</p><h2>{copy.documents.title}</h2></div><p>{copy.documents.text}</p></div><div className="download-grid">{copy.documents.items.map((item)=><article className="download-card" key={item.title}><MaterialIcon name="download" /><b>{item.title}</b><small>{copy.documents.sourceLabel}</small><a href={item.href} target="_blank" rel="noopener noreferrer">{copy.documents.open}</a></article>)}</div></div></section>
      <section className="section"><div className="shell"><div className="section-heading section-heading--center"><p className="kicker">{copy.topics.kicker}</p><h2>{copy.topics.title}</h2></div><div className="accordion">{copy.topics.items.map((item,index)=><details open={index===0} key={item.title}><summary>{item.title}</summary><div>{item.text}</div></details>)}</div></div></section>
    </>
  );
}
...
function MediaPage({ locale }: PageProps) {
  ...
  return (
    <>
      ...
      <section className="section"><div className="shell"><div className="section-heading"><div><p className="kicker">{copy.accreditation.kicker}</p><h2>{copy.accreditation.title}</h2></div><p>{copy.accreditation.text}</p></div><div className="card-grid">{copy.accreditation.cards.map((card,index)=><InfoCard accent={index===2} number={`0${index+1}`} title={card.title} text={card.text} key={card.title}/>)}</div></div></section>
      <section className="section section--soft"><div className="shell split"><div><p className="kicker">{copy.criteria.kicker}</p><div className="accordion">{copy.criteria.items.map((item,index)=><details open={index===0} key={item.title}><summary>{item.title}</summary><div>{item.text}</div></details>)}</div></div><div className="media-frame" style={backgroundImage("/assets/tickets-frame.jpg")}/></div></section>
      <section className="section"><div className="shell"><div className="section-heading section-heading--center"><p className="kicker">{copy.commitment.kicker}</p><h2>{copy.commitment.title}</h2></div><div className="stats-row">{copy.commitment.stats.map((stat)=><div className="stat" key={stat.label}><b>{stat.value}</b><span>{stat.label}</span></div>)}</div></div></section>
...
function ContactPage({ locale }: PageProps) {
  ...
  return (
    <>
      ...
      <section className="section section--soft"><div className="shell"><div className="section-heading section-heading--center"><p className="kicker">{copy.routes.kicker}</p><h2>{copy.routes.title}</h2></div><div className="card-grid card-grid--4">{copy.routes.cards.map((card,index)=><InfoCard accent={index===3} number={`0${index+1}`} title={card.title} text={card.text} key={card.title}/>)}</div></div></section>
    </>
  );
}
...
export function PublicPageContent({
  ...
  const props = { locale, detail, legalTab, event, managedExhibitors, floorPlan };
  switch (page) {

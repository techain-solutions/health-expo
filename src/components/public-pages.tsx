...
function ExhibitorsPage({ locale, managedExhibitors, sector }: PageProps & { managedExhibitors?: PublicExhibitor[] }) {
  ...
  return (
    <>
      ...
      <section className="section section--exhibitor-directory" id="directory"><div className="shell"><ExhibitorDirectory exhibitors={directory} initialSector={sector} initialSectorLabel={sectorIndex >= 0 ? sectorLabels[sectorIndex] : undefined} sectorLabels={sectorLabels} locale={locale} suggestedCategories={copy.suggestedCategories} labels={{ search: copy.searchLabel, placeholder: copy.searchPlaceholder, all: copy.filterAll, empty: copy.empty, noMatches: copy.noMatches, resultCount: copy.resultCount }} /></div></section>
      <CtaBand locale={locale} title={copy.cta.title} text={copy.cta.text} page="participate" label={copy.cta.label} />
    </>
  );
}

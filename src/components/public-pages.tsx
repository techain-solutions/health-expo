...
function ExhibitorDetailPage({ locale, detail, managedExhibitors }: PageProps & { managedExhibitors?: PublicExhibitor[] }) {
  ...
  return (
    <>
      ...
      <section className="section"><div className="shell split"><div className="exhibitor-detail-logo">{matched.imageUrl ? <img src={matched.imageUrl} alt={`${matched.name} exhibitor`} /> : <span className={`fake-logo${matched.style ? ` fake-logo--${matched.style}` : ""}`}>{matched.letter}</span>}</div><div className="split-copy"><p className="kicker">{copy.exhibitorDetail.about.kicker}</p><h2>{copy.exhibitorDetail.about.title}</h2><p>{matched.text}</p><ul className="detail-list"><li><b>{copy.exhibitorDetail.categoryLabel}</b><span>{matched.category}</span></li>{matched.websiteUrl ? <li><b>{copy.exhibitorDetail.websiteLabel}</b><a href={matched.websiteUrl} target="_blank" rel="noopener noreferrer">{copy.common.visitWebsite}</a></li> : null}</ul></div></div></section>

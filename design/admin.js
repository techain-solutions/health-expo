...
function dashboard() {
  const cards = [
    ["storefront", "Exhibitors", "Add, update or remove participating exhibitors.", "exhibitors"],
    ["event", "Event information", "Dates, location, opening hours and visitor information.", "event"],
    ["perm_media", "Photos & videos", "Manage the approved visual media used on the website.", "media"],
    ["inbox", "Form requests", "Review submissions received through the website forms.", "requests"],
    ["open_in_new", "Ticketing link", "Set the external link used by ticket buttons.", "ticketing"]
  ];
  const visibleCards = cards.filter(([, , , page]) => (rolePermissions[rolePreview.value] || rolePermissions.administrator).includes(page));
  return `<div class="admin-page">${pageHead("Administration panel", "A focused workspace for the agreed Health & Beauty Expo website scope.", `<a class="admin-btn admin-btn--outline" href="index.html?page=home" target="_blank">Preview website ${icon("open_in_new")}</a>`)}

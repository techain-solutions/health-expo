...
function homePage() {
  return `<section class="hero hero--cinematic">
    ...
    <div class="shell hero-cinematic__content">
      <div class="hero-copy reveal">
        <p class="eyebrow">Rijswijk 2026 · International medical tourism exhibition</p>
        ...
        <div class="event-meta">
          ...
          <div><span class="meta-icon material-symbols-rounded" aria-hidden="true">location_on</span><p><b>De Broodfabriek</b><small>Rijswijk, NL</small></p></div>
        </div>
      </div>
  ...
  <section class="section"><div class="shell">
    ...
    <div class="card-grid card-grid--4">
      ...
      <article class="info-card reveal"><span class="info-card__number">03</span><h3>Use Fair Match</h3><p>Tell our organizing team what you are looking for and request an introduction.</p><a href="?page=fair-match">Make a request ${icons.arrow}</a></article>
  ...
  <section class="section section--soft"><div class="shell">
    <div class="section-heading reveal"><div><p class="kicker">FEATURED EXHIBITORS</p><h2>Clinics, brands and partners joining Rijswijk 2026.</h2></div><a class="button button--outline" href="?page=exhibitors">View all exhibitors ${icons.arrow}</a></div>
    <div class="exhibitor-grid">${exhibitorCards.slice(0, 3).map(exhibitorCard).join("")}</div>
    <p class="demo-note">Exhibitor records shown here are illustrative. Final exhibitors are supplied and approved by the organiser.</p>
  </div></section>
  ...
  <section class="section section--dark"><div class="shell split">
    <div class="split-copy reveal"><p class="kicker">FAIR MATCH</p><h2 style="color:white">Meet the right clinic, coordinated by our team.</h2><p style="color:rgba(255,255,255,.65)">Review the participating clinics, choose one and propose a preferred date and time. Our organizing team confirms every meeting personally.</p><a class="button button--white" href="?page=fair-match">How Fair Match works ${icons.arrow}</a></div>
    <ol class="timeline reveal"><li><span>01</span><div><h3 style="color:white">Review the clinics</h3><p style="color:rgba(255,255,255,.58)">Browse the participating clinics shown on the site.</p></div></li><li><span>02</span><div><h3 style="color:white">Choose one & propose a time</h3><p style="color:rgba(255,255,255,.58)">Select a clinic and share your preferred date and time.</p></div></li><li><span>03</span><div><h3 style="color:white">The team confirms manually</h3><p style="color:rgba(255,255,255,.58)">We confirm, reschedule or decline and follow up directly.</p></div></li></ol>
  </div></section>
...
function exhibitorsPage() {
  const cards = exhibitorCards.map(exhibitorCard).join("");
...
const fairMatchClinics = exhibitorCards.slice(0, 5);

function fairMatchPage() {
  const clinicCards = fairMatchClinics.map(([letter, style, name, category, text]) => `<article class="clinic-card reveal"><div class="clinic-card__head"><span class="fake-logo${style ? ` fake-logo--${style}` : ""}">${letter}</span><div><small>${category}</small><h3>${name}</h3></div></div><p>${text}</p><button class="button button--outline button--small clinic-select" type="button" data-clinic="${name}">Request a meeting ${icons.arrow}</button></article>`).join("");
  const clinicOptions = fairMatchClinics.map(([, , name]) => `<option>${name}</option>`).join("");
  const timeOptions = Array.from({ length: 16 }, (_, index) => { const minutes = 600 + index * 30; const hh = String(Math.floor(minutes / 60)).padStart(2, "0"); const mm = String(minutes % 60).padStart(2, "0"); return `<option>${hh}:${mm}</option>`; }).join("");
  const fields = `<div class="form-row">${formField("First name", "text", "Your first name")}${formField("Last name", "text", "Your last name")}</div>
    <div class="form-row">${formField("Email", "email", "name@example.com")}${formField("Phone", "tel", "+31 …")}</div>
    <div class="field"><label>Choose a clinic <span>*</span></label><select id="clinicSelect" required><option value="">Select a participating clinic</option>${clinicOptions}</select></div>
    <div class="form-row"><div class="field"><label>Preferred date <span>*</span></label><input type="date" required min="2026-10-24" max="2026-10-25" value="2026-10-24" /><small class="field-hint">Meetings take place during the expo, 24–25 October 2026.</small></div><div class="field"><label>Preferred time <span>*</span></label><select required><option value="">Select a time slot</option>${timeOptions}</select></div></div>
    ${formField("Message (optional)", "textarea", "Add anything that helps the clinic and organizing team prepare for the meeting.")}`;
  return `${pageHero("Fair Match—request a meeting with a clinic.", "Review the participating clinics, choose the one you want to meet, and propose a preferred date and time. The organizing team confirms every request personally.", `<a class="button button--white" href="#request">Request a meeting ${icons.arrow}</a>`)}
  <section class="section"><div class="shell"><div class="section-heading section-heading--center reveal"><p class="kicker">HOW IT WORKS</p><h2>A clear, personal request.</h2><p>Fair Match is a manual appointment request handled by the organizing team. It does not provide automated matching, a live booking calendar, real-time availability, clinic dashboards, chat or calendar synchronisation.</p></div><div class="card-grid"><article class="info-card reveal"><span class="info-card__number">01</span><h3>Review the clinics</h3><p>Browse the participating clinics presented on this page.</p></article><article class="info-card reveal"><span class="info-card__number">02</span><h3>Choose one & propose a time</h3><p>Select a clinic and share your preferred date, time and contact details.</p></article><article class="info-card info-card--teal reveal"><span class="info-card__number">03</span><h3>The team confirms manually</h3><p>The organizing team confirms, proposes another time, or declines and follows up directly.</p></article></div></div></section>
  <section class="section section--soft"><div class="shell"><div class="section-heading reveal"><div><p class="kicker">PARTICIPATING CLINICS</p><h2>Who you can meet.</h2></div><p>Select a clinic below to start your request. Clinic records shown here are illustrative and are supplied and approved by the organiser.</p></div><div class="clinic-grid">${clinicCards}</div></div></section>
  <section class="section" id="request"><div class="shell form-shell"><div class="form-intro"><p class="kicker">MEETING REQUEST</p><h2>Request your meeting.</h2><p>Complete the static preview to demonstrate the intended experience.</p><div class="contact-card"><b>Manual confirmation</b><span>Your request reaches the organizing team by dashboard and email. They confirm, reschedule or decline by hand—there is no automatic booking.</span></div></div>${staticForm("Your meeting request", fields, "Send request")}</div></section>`;
}
...
function participatePage() {
  const packages = [
    ...
    ["Stand 3", "15 m²", "€4,995", ["5 × 3 metre stand", "230V electricity", "2 tables, low or high", "6 chairs, low or high"], ""],
    ["Stand 4", "30 m²", "€6,995", ["6 × 5 metre stand", "230V electricity", "2 tables, low or high", "6 chairs, low or high"], ""]
  ];
...
function participantInfoPage() {
  ...
  <section class="section section--soft"><div class="shell"><div class="section-heading"><div><p class="kicker">DOCUMENTS</p><h2>Downloads for participating companies.</h2></div><p>These documents remain subject to organiser and legal validation before public production publication.</p></div><div class="download-grid"><article class="download-card"><span class="material-symbols-rounded" aria-hidden="true">download</span><b>Participant manual 2026</b><small>Rules, logistics, payment and safety</small><a href="downloads/participant-manual-2026.pdf" target="_blank">Download NL PDF</a><a href="downloads/participant-manual-2026-en.pdf" target="_blank">Download EN PDF</a></article><article class="download-card"><span class="material-symbols-rounded" aria-hidden="true">download</span><b>Exhibitor agreement</b><small>Dutch agreement template</small><a href="downloads/exhibitor-agreement-nl.pdf" target="_blank">Open PDF</a></article><article class="download-card"><span class="material-symbols-rounded" aria-hidden="true">download</span><b>Stand price list</b><small>Stand and sponsorship packages</small><a href="downloads/stand-pricing-2026.pdf" target="_blank">Download PDF</a></article><article class="download-card"><span class="material-symbols-rounded" aria-hidden="true">download</span><b>Expo floor plan</b><small>Static hall layout</small><a href="downloads/expo-floor-plan.pdf" target="_blank">Open PDF</a></article></div></div></section>
...
function contactPage() {
  ...
  const mapQuery = encodeURIComponent("De Broodfabriek, Volmerlaan 12, 2288 GD Rijswijk");
  ...
  <section class="section"><div class="shell form-shell"><div class="form-intro"><p class="kicker">CONTACT</p><h2>We’re here to help.</h2><p>Reach the Health & Beauty Expo team by email, phone or the form.</p><ul class="detail-list"><li><b>General</b><span>info@expofuar.nl</span></li><li><b>Telephone</b><span>+31 (0)570 238 100</span></li><li><b>Office hours</b><span>Mon–Fri 09:00–17:00 · Sat 11:00–16:00</span></li><li><b>Event venue</b><span>De Broodfabriek, Volmerlaan 12, 2288 GD Rijswijk</span></li></ul><p class="validation-banner">Please confirm the public contact details, request recipients and social links before launch.</p></div>${staticForm("Send a message", fields, "Send message")}</div></section>
  <section class="section section--soft"><div class="shell"><div class="section-heading"><div><p class="kicker">FIND US</p><h2>De Broodfabriek, Rijswijk.</h2></div><a class="button button--outline" href="https://maps.google.com/?q=${mapQuery}" target="_blank" rel="noopener">Open in Google Maps ${icons.external}</a></div><div class="map-card reveal"><div class="map-card__pin material-symbols-rounded" aria-hidden="true">location_on</div><div class="map-card__body"><b>De Broodfabriek</b><span>Volmerlaan 12, 2288 GD Rijswijk</span><span>The Netherlands</span></div><a class="map-card__link" href="https://maps.google.com/?q=${mapQuery}" target="_blank" rel="noopener">View directions ${icons.arrow}</a></div></div></section>
  <section class="section section--soft"><div class="shell"><div class="section-heading section-heading--center"><p class="kicker">DIRECT YOUR REQUEST</p><h2>Choose the right route.</h2></div><div class="card-grid card-grid--4"><article class="info-card"><span class="info-card__number">01</span><h3>Visitor</h3><p>Tickets, venue and practical information.</p><a href="?page=visit">Visit information ${icons.arrow}</a></article><article class="info-card"><span class="info-card__number">02</span><h3>Exhibitor</h3><p>Stand, sponsorship and participation options.</p><a href="?page=participate">Exhibit with us ${icons.arrow}</a></article><article class="info-card"><span class="info-card__number">03</span><h3>Media</h3><p>Press, journalist and influencer accreditation.</p><a href="?page=media">Media criteria ${icons.arrow}</a></article><article class="info-card info-card--teal"><span class="info-card__number">04</span><h3>Fair Match</h3><p>Personal introduction request and organizing-team follow-up.</p><a href="?page=fair-match">Open Fair Match ${icons.arrow}</a></article></div></div></section>`;
}
...
const translationSets = {
  nl: {
    ...
    "Documents": "Documenten", "Exhibitor manual (EN)": "Handleiding (EN)", "Participant manual (NL)": "Deelnemershandleiding (NL)", "Exhibitor agreement": "Overeenkomst", "Stand price list": "Standprijzen", "Floor plan PDF": "Plattegrond PDF", "General terms": "Algemene voorwaarden",
  ...
  tr: {
    ...
    "Documents": "Belgeler", "Exhibitor manual (EN)": "Katılımcı rehberi (EN)", "Participant manual (NL)": "Katılımcı rehberi (NL)", "Exhibitor agreement": "Katılım sözleşmesi", "Stand price list": "Stant fiyat listesi", "Floor plan PDF": "Fuar planı PDF", "General terms": "Genel şartlar",
  ...
  ru: {
    ...
    "Documents": "Документы", "Exhibitor manual (EN)": "Руководство (EN)", "Participant manual (NL)": "Руководство (NL)", "Exhibitor agreement": "Соглашение участника", "Stand price list": "Прайс-лист стендов", "Floor plan PDF": "План выставки PDF", "General terms": "Общие условия",
  ...
  ar: {
    ...
    "Documents": "المستندات", "Exhibitor manual (EN)": "دليل العارض (EN)", "Participant manual (NL)": "دليل المشاركين (NL)", "Exhibitor agreement": "اتفاقية العارض", "Stand price list": "قائمة أسعار الأجنحة", "Floor plan PDF": "مخطط المعرض PDF", "General terms": "الشروط العامة",
...
function wirePage() {
  ...
  document.querySelectorAll(".clinic-select").forEach(button => button.addEventListener("click", () => {
    document.querySelector("#request")?.scrollIntoView({ behavior: "smooth" });
    const select = document.querySelector("#clinicSelect");
    if (select && [...select.options].some(option => option.text === button.dataset.clinic)) select.value = button.dataset.clinic;
  }));
